...
import {
  useTVControl,
  CHANNEL_EDITOR_ITEMS_LIST,
  SETTINGS_MENU_ITEMS,
  INSTALL_MENU_ITEMS,
  LANGUAGE_SETTINGS_ITEMS,
  AB_SETTINGS_ITEMS,
  ACCESS_CARD_ITEMS,
} from "../context/TVControlContext";
...
import OpenboxTVInterface from "./OpenboxTVInterface";
...
const IOSSettingsIcon = (
  ...
  >
    <FaCog
      color="#666"
      size={24}
      style={{ filter: "drop-shadow(0 1px 1px #fff8)" }}
    />
  </span>
);
...
const apps = [
  {
    ...
    icon: (
      ...
      >
        <FaTv color="#2196f3" size={70} />
        <FaMagic color="#9c27b0" size={54} />
      </div>
    ),
  },
  {
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <span
              style={{
                ...
                color: "#fff",
                letterSpacing: 0.5,
                textShadow: "0 2px 12px #3386ff88",
              }}
            >
              Conax Information
            </span>
        ...
        >
          ...
          >
            {[
              ...
              >
                <span style={{ flex: 1, textAlign: "left", paddingLeft: 32 }}>
                  {label}
                </span>
                <span style={{ flex: 1, textAlign: "right", paddingRight: 32 }}>
                  {value}
                </span>
              </div>
            ))}
...
function ChannelSearchProgressModal({
  progress,
  tvList,
  radioList,
  onClose,
  ...
  React.useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      const radioCount = Math.floor(
        (nextProgress / 100) * RADIO_CHANNELS.length,
      );

      const nextTV = DEFAULT_CHANNEL_LIST.slice(0, tvCount).map(
        (ch) => ch.name,
      );
      const nextRadio = RADIO_CHANNELS.slice(0, radioCount);

      window.dispatchEvent(
        new CustomEvent("channel-search-progress", {
          detail: { progress: nextProgress, tv: nextTV, radio: nextRadio },
        }),
      );
    }, 100); // 100мс на тик
  ...
  React.useEffect(() => {
    if (tvListContainerRef.current && tvList.length > 0) {
      tvListContainerRef.current.scrollTop =
        tvListContainerRef.current.scrollHeight;
    }
    if (radioListContainerRef.current && radioList.length > 0) {
      radioListContainerRef.current.scrollTop =
        radioListContainerRef.current.scrollHeight;
    }
  ...
  return (
    ...
    >
        ...
        >
          ...
          >
            {tvList.map((ch, i) => (
              ...
              >
                <span style={{ width: 38, textAlign: "right", marginRight: 8 }}>
                  {i + 1}
                </span>
                <span
                  style={{
        ...
        >
          ...
          >
            {radioList.map((ch, i) => (
              ...
              >
                <span style={{ width: 22, textAlign: "right", marginRight: 8 }}>
                  {i + 1}
                </span>
                <span
                  style={{
      ...
      >
        ...
        >
          <span style={{ color: "#fff", fontSize: 14, width: 90 }}>
            Уровень
          </span>
          <SolidBar
            percent={77}
      ...
      >
        <div style={{ display: "flex", alignItems: "center", marginTop: 6 }}>
          <span style={{ color: "#fff", fontSize: 15, width: 90 }}>
            Процесс
          </span>
          <SegmentedBar
            percent={progress}
...
export default function TVScreen({
  ...
  React.useEffect(() => {
    ...
    return () =>
      window.removeEventListener("virtual-remote-exit", onVirtualExit);
  ...
  const showChannelEditorHint =
    highlight &&
    highlight.step === 0 &&
    highlight.errorKey === "channel-editor" &&
    highlight.subKey === "delete";
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("channel-search-progress", onProgress);
  }, [sendCommand]);
  ...
  return (
    <RealisticTVFrame width={width} height={height}>
      ...
      >
        {/* Если выбрана приставка Openbox, показываем специальный интерфейс */}
        {deviceId === "openbox" ? (
          <OpenboxTVInterface />
        ) : (
          <>
                  ...
                  >
                    ...
                    >
                          ...
                          {[1, 2, 3, 4, 5].map((num, idx) => (
                            ...
                            >
                              ...
                              >
                                ...
                                <span style={{ fontSize: 15, marginRight: 3 }}>
                                  {["️", "↔️", "⏭️", "🔒", "★"][idx]}
                                </span>
                                <span
                                  style={{
                        ...
                        >
                          {tvState.channelList.map((ch, idx) => (
                            <div
                              ...
                              style={{
                                background:
                                  idx === tvState.selectedChannelIndex
                                    ? "#e048b1"
                                    : "transparent",
                                color:
                                  idx === tvState.selectedChannelIndex
                            ...
                            >
                              ...
                              {tvState.activePanelBtn === 1 &&
                                tvState.channelsToDelete.has(idx) && (
                                  <span
                                    style={{
                                      position: "absolute",
                                      right: 8,
                              ...
                              {tvState.activePanelBtn === 2 &&
                                tvState.channelsToMove.has(idx) && (
                                  <span
                                    style={{
                                      position: "absolute",
                                      right: 8,
                              ...
                              {tvState.activePanelBtn === 3 &&
                                tvState.channelsToSkip.has(idx) && (
                                  <span
                                    style={{
                                      position: "absolute",
                                      right: 8,
                              ...
                              {tvState.activePanelBtn === 4 &&
                                tvState.channelsToLock.has(idx) && (
                                  <span
                                    style={{
                                      position: "absolute",
                                      right: 8,
                              ...
                              {/* Индикатор уже избран��ых каналов */}
                              {tvState.favoriteChannels.has(idx) && (
                                <span
                                  style={{
                                    position: "absolute",
                                    right:
                                      tvState.activePanelBtn === 5 ? 24 : 8,
                                    top: "50%",
                                    transform: "translateY(-50%)",
                                    width: 12,
                                    height: 12,
                      ...
                      >
                        ...
                        >
                          ...
                          <div
                            style={{
                              ...
                              height: "100%",
                              background:
                                "linear-gradient(90deg, #e048b1 0%, #222 100%)",
                              display: "flex",
                              alignItems: "center",
                              justifyContent: "center",
                            }}
                        ...
                        >
                          {tvState.channelList[tvState.selectedChannelIndex]
                            ?.info || "Нет информации"}
                        </div>
                      </div>
                    </div>
                ...
                >
                  <div
                    style={{
                      display: "flex",
                      alignItems: "center",
                      gap: tvState.activePanelBtn === 1 ? 0 : 152,
                    }}
                  >
                    <span
                ...
                {CHANNEL_EDITOR_ITEMS_LIST.map((item, idx) => {
                  ...
                  return (
                    <div
                      ...
                      style={{
                        background:
                          idx === tvState.channelEditorIndex
                            ? "#e048b1"
                            : "transparent",
                        color:
                          idx === tvState.channelEditorIndex ? "#fff" : "#fff",
                        fontWeight:
                          idx === tvState.channelEditorIndex ? 700 : 400,
                        fontSize: 14,
                        padding: "7px 16px",
                        borderRadius: 7,
                        margin: "2px 7px",
                        ...
                        boxShadow: showYellow
                          ? "0 0 16px 2px #ffd60088, 0 2px 8px #ffd60055"
                          : undefined,
                        animation: showYellow
                          ? "yellow-blink 1.1s infinite alternate"
                          : undefined,
                      }}
                    >
                      {item}
              ...
              >
                ...
                {SETTINGS_MENU_ITEMS.slice(
                  Math.max(
                    0,
                    Math.min(
                      tvState.settingsModalIndex - 2,
                      SETTINGS_MENU_ITEMS.length - 5,
                    ),
                  ),
                  Math.max(
                    0,
                  ...
                  return (
                    <div
                      ...
                      style={{
                        background:
                          globalIdx === tvState.settingsModalIndex
                            ? "#e048b1"
                            : "transparent",
                        color:
                          globalIdx === tvState.settingsModalIndex
                            ...
                            : "#fff",
                        fontWeight:
                          globalIdx === tvState.settingsModalIndex ? 700 : 400,
                        fontSize: 14,
                        padding: "7px 16px",
                        borderRadius: 7,
                        margin: "2px 7px",
                ...
                {/* Стрелка вниз */}
                {tvState.settingsModalIndex <
                  SETTINGS_MENU_ITEMS.length - 1 && (
                  <div
                    style={{
                      textAlign: "center",
                      color: "#fff",
              ...
              >
                ...
                {INSTALL_MENU_ITEMS.slice(
                  Math.max(
                    0,
                    Math.min(
                      tvState.installModalIndex - 2,
                      INSTALL_MENU_ITEMS.length - 5,
                    ),
                  ),
                  Math.max(
                    0,
                  ...
                  return (
                    <div
                      ...
                      style={{
                        background:
                          globalIdx === tvState.installModalIndex
                            ? "#e048b1"
                            : "transparent",
                        color:
                          globalIdx === tvState.installModalIndex
                            ...
                            : "#fff",
                        fontWeight:
                          globalIdx === tvState.installModalIndex ? 700 : 400,
                        fontSize: 14,
                        padding: "7px 16px",
                        borderRadius: 7,
                        margin: "2px 7px",
            ...
            {tvState.languageSettingsModalOpen && (
              <ChannelListModal
                ...
                onSelect={(idx) =>
                  sendCommand(
                    idx > tvState.languageSettingsModalIndex ? "down" : "up",
                  )
                }
            ...
            {tvState.abSettingsModalOpen && (
              <ChannelListModal
                ...
                onSelect={(idx) =>
                  sendCommand(
                    idx > tvState.abSettingsModalIndex ? "down" : "up",
                  )
                }
                onClose={() => sendCommand("exit")}
                onValueChange={(idx, dir) => sendCommand(dir)}
            ...
            {tvState.accessCardModalOpen && (
              <ChannelListModal
                ...
                onSelect={(idx) =>
                  sendCommand(
                    idx > tvState.accessCardModalIndex ? "down" : "up",
                  )
                }
                onClose={() => sendCommand("exit")}
                oneColumn={true}
              ...
              <>
                <ChannelListModal
                  ...
                  signalBlock={
                    <>
                      ...
                      >
                        <span
                          style={{ color: "#fff", fontSize: 13, width: 90 }}
                        >
                          Уровень
                        </span>
                        <div
                        ...
                        >
                          <div
                            style={{
                              ...
                              height: "100%",
                              background:
                                "linear-gradient(90deg, #1e90ff 60%, #00eaff 100%)",
                              borderRadius: 6,
                            }}
                          />
                        </div>
                        ...
                        >
                          <div
                            style={{
                              ...
                              height: "100%",
                              background:
                                "linear-gradient(90deg, #b6ff00 60%, #3fcf4a 100%)",
                              borderRadius: 6,
                            }}
                          />
                        </div>
                ...
                >
                  <span style={{ marginRight: 32 }}>
                    <span style={{ color: "#ff1744" }}>●</span> Список спутник
                  </span>
                  <span style={{ marginRight: 32 }}>
                    <span style={{ color: "#00e676" }}>●</span> Список транспо
                  </span>
                  <span>
                    <span style={{ color: "#2979ff" }}>●</span> Поиск T2-MI
                  </span>
                </div>
              </>
            )}
            ...
            {tvState.searchSettingsModalOpen && (
              <div
                style={{
                  ...
                  maxWidth: 370,
                  background:
                    "linear-gradient(180deg, #f8f8f8 80%, #e0e0e0 100%)",
              ...
              >
                ...
                <div
                  style={{
                    background:
                      "linear-gradient(90deg, #e0e0e0 60%, #f8f8f8 100%)",
                ...
                >
                  {SEARCH_SETTINGS_ITEMS.map((item, idx) => (
                    <div
                      ...
                      onClick={() =>
                        sendCommand(
                          idx > tvState.searchSettingsModalIndex
                            ? "down"
                            : idx < tvState.searchSettingsModalIndex
                              ? "up"
                              : null,
                      ...
                      style={{
                        ...
                        color:
                          idx === tvState.searchSettingsModalIndex
                            ...
                            : "#222",
                        fontWeight:
                          idx === tvState.searchSettingsModalIndex ? 700 : 400,
                        fontSize: 15,
                        borderRadius: 5,
                        margin: "1.5px 0",
                        minHeight: 28,
                  ...
                  <div
                    ...
                    style={{
                      ...
                      justifyContent: "center",
                      background:
                        tvState.searchSettingsModalIndex ===
                        SEARCH_SETTINGS_ITEMS.length
                          ? "#2979ff"
                          : "transparent",
                      color:
                        tvState.searchSettingsModalIndex ===
              ...
              {apps.map((app, i) => (
                <div
                  ...
                  style={{
                    ...
                    outline:
                      highlight?.type === "menu" &&
                      highlight?.key === "channel-editor" &&
                      app.name === "Редактор каналов"
                        ? "3px solid #ffd600"
                        : undefined,
                    boxShadow:
                      (tvState.selectedIcon === i
                        ? "0 0 0 3px #e048b1, 0 0 16px 2px #e048b1aa"
                        : "") +
                        (highlight?.type === "menu" &&
                        highlight?.key === "channel-editor" &&
                        app.name === "Редактор каналов"
                          ? ", 0 0 16px 4px #ffd60088"
                          : "") || undefined,
                  }}
                >
            ...
            {showDeleteModal && (
              <div
                style={{
                  ...
                  zIndex: 100,
                  background:
                    "radial-gradient(ellipse at 60% 40%, #2563eb55 0%, #181c20cc 100%)",
              ...
              >
                <div
                  style={{
                    background:
                      "linear-gradient(135deg, #23272e 60%, #181c20 100%)",
                ...
                >
                  <div
                    style={{
                      ...
                      marginBottom: 18,
                      letterSpacing: 0.5,
                      textShadow: "0 2px 12px #00eaff88",
                      marginTop: 18,
                    }}
                  >
                    Подтверждение
                  ...
                  <div style={{ display: "flex", gap: 28, marginTop: 12 }}>
                    ...
                    <button
                      ...
                      style={{
                        background:
                          "linear-gradient(135deg, #ff1744 60%, #b71c1c 100%)",
                        color: "#fff",
                  ...
                  <div
                    style={{
                      ...
                      opacity: 0.35,
                      background:
                        "radial-gradient(ellipse at center, #00eaff 0%, #2563eb 60%, transparent 100%)",
                    }}
                  />
                </div>
                <style>{`
